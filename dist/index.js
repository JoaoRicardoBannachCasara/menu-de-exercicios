...
// Importação do módulo readline-sync para entrada de dados
const readline_sync_1 = __importDefault(require("readline-sync"));
function exercicio1() {
    const a = readline_sync_1.default.questionInt('Digite o primeiro número: ');
    const b = readline_sync_1.default.questionInt('Digite o segundo número: ');
    console.log(`Soma: ${a + b}`);
}
function exercicio2() {
    const n = readline_sync_1.default.questionInt('Digite um número: ');
    console.log(n % 2 === 0 ? 'Par' : 'Ímpar');
}
function exercicio3() {
    const n1 = readline_sync_1.default.questionFloat('Nota 1: ');
    const n2 = readline_sync_1.default.questionFloat('Nota 2: ');
    const n3 = readline_sync_1.default.questionFloat('Nota 3: ');
    console.log(`Média: ${(n1 + n2 + n3) / 3}`);
}
function exercicio4() {
    const c = readline_sync_1.default.questionFloat('Temperatura em Celsius: ');
    const f = (c * 9) / 5 + 32;
...
function exercicio6() {
    const arr = [];
    for (let i = 0; i < 5; i++) {
        arr.push(readline_sync_1.default.questionInt(`Digite o ${i + 1}º número: `));
    }
...
function exercicio8() {
    const str = readline_sync_1.default.question('Digite uma string: ').toLowerCase();
    const count = str.split('').filter((c) => 'aeiou'.includes(c)).length;
    console.log(`Numero de vogais: ${count}`);
}
function exercicio9() {
    const a = readline_sync_1.default.questionFloat('Número 1: ');
    const op = readline_sync_1.default.question('Operador (+ - * /): ');
    const b = readline_sync_1.default.questionFloat('Número 2: ');
    let r;
    switch (op) {
        case '+':
            r = a + b;
...
function exercicio11() {
    ...
    const idade = readline_sync_1.default.questionInt('Idade: ');
...
function exercicio12() {
    const nome = readline_sync_1.default.question('Nome: ');
    const idade = readline_sync_1.default.questionInt('Idade: ');
    const matricula = readline_sync_1.default.question('Matricula: ');
...
function exercicio14() {
    const n = readline_sync_1.default.questionInt('Digite um numero: ');
    for (let i = 1; i <= 10; i++) {
        console.log(`${n} x ${i} = ${n * i}`);
    }
}
function exercicio15() {
    const peso = readline_sync_1.default.questionFloat('Peso (kg): ');
    const altura = readline_sync_1.default.questionFloat('Altura (m): ');
...
function exercicio17() {
    ...
    let tentativa;
    do {
        tentativa = readline_sync_1.default.questionInt('Adivinhe o número (1-100): ');
...
function menu() {
    ...
    do {
        ...
        console.log('0 - Sair');
        opcao = readline_sync_1.default.questionInt('Escolha uma opcao: ');
